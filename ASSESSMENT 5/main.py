...
#5th question sort product by category then by price
@app.get('/products/sort-by-category')
def sort_category_price():
    new_sort_products = sorted(products,key=lambda x: (x['category'],x['price']))
    return {'products':new_sort_products,'total':len(new_sort_products)}

#6th question all sort, search, pagination
@app.get('/products/browse') 
def browse_products( 
    keyword: str = Query(None,description="Enter Keyword to search"), sort_by: str = Query('price',description="Sort by price(default) followed by name"), order: str = Query('asc',description="Order in ascending order by deafult"), page: int = Query(1, ge=1,description="Number of page"), limit: int = Query(4, ge=1, le=20,description="Limit per page"),
    ): 
    # Step 1: Search 
    result = products 
    if keyword: 
...
#4th question Search the Orders List
@app.get('/orders/search')
def search_products(
    customer_name: str = Query(..., description='Enter Customer name to fetch orders data'),
):
    results = [
        o for o in orders
    ...
    if not results:
        return {'message': f'No products found for: {customer_name}', 'results': []}
    return {
